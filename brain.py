"""
The "Brain" class handles most of celestai's processing.
...
from celestai import settings, stt, tts, apis, mods, log
#from celestai import discord_link as dislink
...
class Brain():
    ...
    def greet(self):
        ...
        if apis.api_lib['user_api'].name():
            print('\n~ Hiya, '+apis.api_lib['user_api'].name()+'!\n')
        else:
            print('\n~ Hello, what can I do for you today?\n')
        print('~ Try asking:')
        print('  - "Celest (double beep) what\'s the weather like in Eq?"')
        print('  - "Celest (double beep) what is the capital of Tanzania?"')
        print('  - "Celest (double beep) open fimfiction.net"\n')

    def greet_discord(self):
        """ Greet Discord"""

        client = self.bot
        print(client)
        channel = client.get_channel(822263148551602268)
        print(channel)

        embedVar = discord.Embed(title="CelestAI", description="", color=0x00ff00)
        embedVar.add_field(name="API's", value=apis.list_apis_discord(), inline=False)

        client.loop.create_task(channel.send(embed = embedVar))
    ...
    def run(self):
        ...
        async def on_message(message):
            if message.content[0] in '~':
                ...
                if message.content == 'celest stop':
