...
from celestai import settings, log, discord_link, brain

pyglet.options['shadow_window']=False # maybe fix errors... MULTITHREADED ERROR
...
def play_mp3(file_name, file_path=settings.MEDIA_DIR):
    ...
    pyglet.clock.schedule_once(exit_callback, music.duration) #sound.duration
    pyglet.app.run()


def speak(phrase, cache=True, filename='default', show_text=True, log_text=True): # cache is normally false
    ...
    if not settings.USE_TTS:
        ...
        #log.info(brain.inst.msg_channel)\
        message = brain.inst.msg
        client = brain.inst.bot
        #await message.channel.send(phrase)
        #discordphrase = phrase
        client.loop.create_task(message.channel.send(phrase))
